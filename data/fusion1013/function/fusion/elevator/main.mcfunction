execute as @a[scores={elevator=..40}] at @s if entity @e[tag=elevator,distance=..2] run scoreboard players add @s elevator 1
execute as @a[scores={elevator=1..}] at @s unless entity @e[tag=elevator,distance=..2] run scoreboard players remove @s elevator 1
